...
object Prefs {
    ...
    fun logLevelUp(context: Context, level: Int, text: String) {
        val key = "levelup_${level}_${System.currentTimeMillis()}"
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .edit().putString(key, text).apply()
    }
    ...
    fun getRandomQuestionEnabled(context: Context): Boolean =
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .getBoolean("random_q_enabled", true)

    fun setRandomQuestionEnabled(context: Context, enabled: Boolean) {
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .edit().putBoolean("random_q_enabled", enabled).apply()
    }

    fun getRandomQuestionHour(context: Context): Int =
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .getInt("random_q_hour", 12)

    fun getRandomQuestionMinute(context: Context): Int =
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .getInt("random_q_minute", 0)

    fun setRandomQuestionTime(context: Context, hour: Int, minute: Int) {
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .edit()
            .putInt("random_q_hour", hour)
            .putInt("random_q_minute", minute)
            .apply()
    }

    fun saveRandomQuestionAnswer(context: Context, dateKey: String, question: String, text: String) {
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
            .edit().putString("random_q_answer_$dateKey", "$question|||$text").apply()
    }

    /** يرجّع (السؤال, الإجابة) لآخر مرة اتسجلت أو null */
    fun getLastRandomQuestionAnswer(context: Context): Pair<String, String>? {
        val prefs = context.getSharedPreferences(NAME, Context.MODE_PRIVATE)
        val keys = prefs.all.keys.filter { it.startsWith("random_q_answer_") }
        val latestKey = keys.maxOrNull() ?: return null
        val raw = prefs.getString(latestKey, "") ?: return null
        val parts = raw.split("|||")
